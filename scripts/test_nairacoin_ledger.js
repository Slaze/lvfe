...
assert(ledger.getBalance("ada") === 0n, "no implicit faucet");
ledger.ensureFaucet("ada");
assert(ledger.getBalance("ada") === 100n * BigInt(protocol.ATOMIC_PER_COIN), "demo faucet 100 whole");
assert(ledger.FAUCET_LABEL === "demo faucet", "labelled faucet");
ledger.ensureFaucet("ada");
assert(ledger.getBalance("ada") === 100n * BigInt(protocol.ATOMIC_PER_COIN), "faucet once");

assert(ledger.debit("ada", 42 * protocol.ATOMIC_PER_COIN) === true, "debit 42 whole as atomic");
assert(ledger.getBalance("ada") === 58n * BigInt(protocol.ATOMIC_PER_COIN), "58 left");
...
assert(ledger.getBalance("ada") === 60n * BigInt(protocol.ATOMIC_PER_COIN), "after whole debit");
assert(ledger.creditWhole("ada", 1_000_000_000) === true, "credit 1e9 whole");
assert(ledger.balanceWhole("ada") === 1_000_000_060, "1e9 + 60");

const keys = JSON.stringify(ledger.loadWallet("ada"), function (_k, v) {
